...
vi.mock("../../../../src/shared/services/emails/create-mail-body.js");
vi.mock("../../../../src/shared/services/emails/send-mail.js");
...
describe("Unit | Authentication | Services | Send mail to activate user service", () => {
  it("should create token, create mail body and send mail", async () => {
    ...
    createMailBody.mockResolvedValue("document");
    ...
    expect(createMailBody).toHaveBeenCalledWith("confirm-register", { firstname: data.firstname, lastname: data.lastname, activateLink: "http://localhost/#/authentication/activate?token=token" });
    expect(sendMail).toHaveBeenCalledWith({ to: data.email, subject: "Confirmation d'inscription", html: "document" });
  });
});
